...
def validate(self, method=None):
    if self.is_new() and 'param' in self.sequence:
        try:
            sql = """SELECT sequence
                     FROM `tab{0}`
                     WHERE company=%s AND sequence IS NOT NULL and YEAR(creation) = %s and sequence like %s
                     ORDER BY creation DESC
                     LIMIT 1""".format(self.doctype)
            last_count = frappe.db.sql(sql, (self.company, get_year(self.posting_date), self.sequence), as_dict=False)
...
def set_name(doc):
    if 'param' in doc.sequence:
        ...
        doc.name = f"{year}{type}{month}{vertical}{last_number}"
